...
export class SaleComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.formAddItem.get("barcode")!.valueChanges.subscribe((barcode) => {
            this.productService
                .findProductByBarcodeActive(barcode)
                .subscribe((product) => {
    ...
    openDialog(): void {
        ...
        if (this.productsSolds.length <= 0) {
            this.snackBar.open(
                "É necessário inserir ao menos um item para realizar a venda venda!",
                "Atenção",
                {
                    duration: 3000,
                }
    ...
    adicionarItem() {
        ...
        this.productService
            .findProductByBarcodeActive(formValues.barcode)
            .subscribe(
                (foundProduct) => {
                    if (foundProduct.stock < formValues.quantidade) {
                        this.snackBar.open(
                            "O produto não tem a quantidade selecionada em estoque",
                            "OK",
                            {
                                duration: 3000,
                            }
                    ...
                    const total = foundProduct.price * formValues.quantidade;
